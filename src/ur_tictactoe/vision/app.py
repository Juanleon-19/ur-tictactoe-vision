...
def run_move_detection(config: VisionConfig, stable_frames: int = 5) -> int:
    """Validate HumanMoveDetector with live camera and ArUco observations."""
    detector = ArucoDetector(config.aruco.dictionary)
    move_detector = HumanMoveDetector(stable_frames=stable_frames)
    occupied_cells: set[int] = set()
    last_human_move: int | None = None
    previous_time = time.perf_counter()
    fps = 0.0
    ...
    try:
        with Camera(config.camera) as camera:
            cv2.namedWindow(config.ui.window_name, cv2.WINDOW_NORMAL)
            print(
                "[INFO] Human move detection running with "
                f"stable_frames={stable_frames}. Press q or Esc to close."
            )
            ...
            while True:
                ...
                _draw_status(display, f"Missing IDs: {missing or 'none'}", 90)
                _draw_status(display, f"FPS: {fps:.1f}", 120)
                if last_human_move is not None:
                    _draw_status(
                        display,
                        f"HUMAN MOVE: CELL {last_human_move}",
                        160,
                    )
